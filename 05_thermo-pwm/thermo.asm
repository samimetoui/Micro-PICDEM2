...
; La vitesse du moteur varie en fonction de la température. Ceci en faisant varier
; le rapport cyclique du signal qui actionne le moteur. Le moteur s'arrête 2 secondes
; apres que la température soit décendue en dessous de la consigne.
;***********************************************************************************
...
start
	...
	movlw 	0x3F			;PWM period =  15,6 kHz PWM ***********
	movwf 	PR2				;
	...
	clrf 	CCPR2L			;msb duty cycle = 0 *****************

	movlw 	0xC 			;select pwm mode, lsb duty cycle = 0 ***********
	movwf 	CCP2CON
...
sw0	
	...
	movf	TMP,W			;********************
	movwf 	CCPR2L			;duty cycle = température
							;modifie vitesse du moteur
